...
const BRAND_ANALYSIS_TEMPLATE_VERSION = "023-chinese-v1";
...
- 每个方案必须是完整中文生图提示词：把客户品牌名（公司名称字段）原样写入提示词并以品牌中文清晰为主视觉；现代简约/扁平/白色背景；明确要求每个字只出现一次、无重复、无多余文字、无错字；禁止 seal stamp/印章/篆书/雕刻/engraved/环形/仿古纹样；模板中的 XXX 必须替换为真实品牌名，不得原样输出 XXX
...
返回严格JSON，不要markdown包裹：
{
  ...
  "logoDesignSuggestions": {
    "note": "IMPORTANT: 四条 prompts 必须全部用中文撰写，每条提示词都必须把客户品牌名（公司名称字段）原样写入并以品牌中文清晰为主视觉；现代简约/扁平/白色背景；必须明确要求每个字只出现一次、无重复、无多余文字、无错字；严禁使用“大字”“粗壮”“横平竖直”等强调放大文字的措辞；严禁 seal stamp、印章、篆书、雕刻、engraved、环形小字、仿古纹样。模板中的 XXX 必须替换为公司名称字段中的真实品牌名，不得原样输出 XXX。禁止把地名或行业词当作品牌标识，品牌名是唯一主角。",
