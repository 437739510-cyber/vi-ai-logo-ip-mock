...
export async function GET(req: NextRequest) {
  try {
    ...
    const brandProfile = clientInfo.brandProfile as Record<string, any> | undefined;
    const logoGenStatus = clientInfo.logoGenerationStatus as Record<string, any> | undefined;
    
    const details = {
      logoGenerated: !!brandProfile?.logoGenerationResults?.length,
      logoSelected: !!clientInfo.selectedLogo,
      manualGenerated: !!clientInfo.manualGenerated,
      pdfGenerated: !!clientInfo.pdfUrl,
      brandProfile: brandProfile ? {
        analysisStatus: brandProfile.analysisStatus || null,
        logoDesignSuggestions: !!brandProfile.logoDesignSuggestions,
      } : null,
      logoGeneration: logoGenStatus ? {
        total: logoGenStatus.total || 0,
        completed: logoGenStatus.completed || 0,
        completedAt: logoGenStatus.completedAt || null,
        failedAt: logoGenStatus.failedAt || null,
      } : null,
    };
