/**
 * TICKET-122-R1: frozen local POLP input, two real DeepSeek layout rounds,
 * deterministic baseline, PPTX structure/media identity evidence.
 *
 * Run only with the ticket-authorized local env loader:
 *   node --env-file=.env.local --import tsx scripts/_ab-real-deepseek-layout-122-r1.ts
 */
...
const BASE_OUT = path.join(ROOT, "logs/122-r1");
...
const plannerInput = Object.freeze({
  ...
  templateId: `__TICKET_122_R1_NO_TEMPLATE_${runId}__`,
  mascotAssets,
  assetAnalysis: {
...
// Patch SupabaseClient before importing the production guard/engine. The local
// chain is thenable and performs no fetch/I/O. Any non-api_usage_log table is fatal.
let usageStubCalls = 0;
let usageStubId = 0;
const originalFrom = SupabaseClient.prototype.from;
SupabaseClient.prototype.from = function localUsageStub(table: string): any {
  if (table !== "api_usage_log") throw new Error(`Blocked Supabase table: ${table}`);
  usageStubCalls += 1;
  let mode = "select";
  const chain: any = {
    select() { return chain; },
    ...
    eq() { return chain; },
    single() { return Promise.resolve({ data: { id: ++usageStubId }, error: null }); },
    then(resolve: (value: unknown) => unknown, reject: (reason: unknown) => unknown) {
      return Promise.resolve(mode === "select" ? { data: [], error: null } : { data: null, error: null }).then(resolve, reject);
    },
  };
  return chain;
} as typeof SupabaseClient.prototype.from;

const originalFetch = globalThis.fetch;
let blockedNetworkRequests = 0;
let deepseekNetworkRequests = 0;
let activeCall: { round: string; pageId: string; attempt: number } | null = null;
...
globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (cumulativeCostCny > MAX_COST_CNY) throw new Error(`COST_CAP_EXCEEDED:${cumulativeCostCny}`);
    }
  } catch (error) {
    record.evidenceError = error instanceof Error ? error.message : String(error);
  }
...
async function injectWayfinding(pptx: Buffer, variant: Variant): Promise<Buffer> {
  ...
  const mediaName = "ppt/media/image-wayfinding-122-r1.png";
  ...
  const rel = `<Relationship Id="rId${nextId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image-wayfinding-122-r1.png"/>`;
  ...
  const pic = `<p:pic><p:nvPicPr><p:cNvPr id="9001" name="Wayfinding122R1"/><p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="rId${nextId}"/><a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="5290000" y="6995000"/><a:ext cx="1280160" cy="1280160"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr></p:pic>`;
...
async function main(): Promise<void> {
  ...
  for (const round of ["A1", "A2"] as const) {
    for (const pageId of TARGET_PAGES) {
      ...
      while (!accepted) {
        ...
        if (cumulativeCostCny >= MAX_COST_CNY) throw new Error(`COST_CAP_REACHED:${cumulativeCostCny}`);
        attempts += 1;
        activeCall = { round, pageId, attempt: attempts };
        const result = await planLayoutEngine({ ...layoutParams, pageId });
        activeCall = null;
  ...
  if (successes !== REQUIRED_SUCCESSES || attempts > MAX_ATTEMPTS || cumulativeCostCny > MAX_COST_CNY) {
  ...
  const evidence = {
    ...
    network: { allowedTarget: EXACT_DEEPSEEK_URL, deepseekNetworkRequests, blockedNetworkRequests },
    production: { supabaseNetworkRequests: 0, storageNetworkRequests: 0, orderNetworkRequests: 0, productionWrites: 0, localUsageStubCalls: usageStubCalls },
    calls: { successes, attempts, cumulativeCostCny, capCny: MAX_COST_CNY, records: callEvidence },
...
void main().catch((error) => {
  ...
  fs.writeFileSync(path.join(RUN_DIR, "fatal.json"), JSON.stringify({ runId, error: message, attempts: deepseekNetworkRequests, cumulativeCostCny, blockedNetworkRequests, callEvidence }, null, 2), "utf8");
  ...
  SupabaseClient.prototype.from = originalFrom;
