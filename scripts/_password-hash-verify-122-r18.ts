// TICKET-122-R18 验证脚本（本地，不触碰生产库）：
// 1) hash -> verify 往返正确；
// 2) isPasswordHash 判别正确；
// 3) 模拟登录懒迁移：新哈希账号直接可登录；旧明文账号登录成功并自动升级为哈希；
//    错误密码不通过、不迁移。
...
async function main(): Promise<void> {
  ...
  console.log("");
  console.log("=== 2) 模拟登录懒迁移（n:1 还原 src/app/api/admin/login/route.ts 分支） ===");
  // 行结构模拟 student_accounts
  interface Row {
    id: string;
  ...
  assert(isPasswordHash(rows[1].password_hash) === true, "懒迁移后该行已是哈希");
  ...
  assert(isPasswordHash(rows[2].password_hash) === false, "失败路径该行仍为明文（未写库）");
