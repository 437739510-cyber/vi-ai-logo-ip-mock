...
// V3: scene keys use page-type prefix matching render-pptx expectation
// Format: {pageType}-{n} — renderScene filters by k.startsWith(type)
const SCENE_KEY_MAP = {
  'stationery-1': { file: 'zhaocai_scene2_storefront_00001_.png', label: '门店招牌', title: '门店招牌应用' },
  'packaging-1':  { file: 'zhaocai_scene2_packaging_00001_.png',   label: '外卖袋/包装盒', title: '包装系统应用' },
  'packaging-2':  { file: 'zhaocai_scene2_tray_00001_.png',         label: '托盘/餐垫', title: '包装系统应用' },
  'marketing-1':  { file: 'zhaocai_scene2_poster_00001_.png',       label: '宣传海报', title: '营销展示应用' },
  'marketing-2':  { file: 'zhaocai_scene2_card_00001_.png',         label: '会员卡', title: '营销展示应用' },
};

console.log('[2/5] Loading scene images...');
const sceneImages = {};
const sceneLabels = {};
const sceneSectionTitles = {};
for (const [key, def] of Object.entries(SCENE_KEY_MAP)) {
  const fp = path.join(OUT, def.file);
  const buf = await fs.readFile(fp);
  sceneImages[key] = 'image/png;base64,' + buf.toString('base64');
  sceneLabels[key] = def.label;
  sceneSectionTitles[key] = def.title;
  console.log('  [OK] ' + key + ' <= ' + def.file + ' (' + (buf.length / 1024).toFixed(0) + ' KB)');
}

console.log('[3/5] Planning pages via DeepSeek...');
...
const options = {
  ...
  compressImages: true,
  brandStory: '南昌开了十年的汉堡炸鸡店，主要服务附近高中和大学学生。从一家小店开始，始终坚持现点现做，用新鲜食材和真诚服务赢得了一届又一届学生的喜爱。招财进堡不只是一家快餐店，更是无数年轻人青春记忆中的味道。',
  fullBrandName: '招财进堡',
  englishName: 'ZHAOCAI JINBAO',
};

console.log('[4/5] Rendering PPTX...');
const buf = await renderPptxToBuffer(blueprints, options);
const fileName = 'vi-manual-VI-20260630-CKJ0-' + Date.now() + '.pptx';
const outPath = path.join(OUT, fileName);
await fs.writeFile(outPath, buf);
console.log('  DONE: ' + outPath + ' (' + (buf.length / 1024).toFixed(0) + ' KB)');

console.log('[5/5] Uploading to Supabase & updating DB...');
const storagePath = PROJECT_ID + '/' + fileName;
await supabase.storage.from('manuals').upload(storagePath, buf, {
  contentType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
  upsert: true,
});
const storageUrl = 'https://fzoscrutqhdfzwnjgjvs.supabase.co/storage/v1/object/public/manuals/' + storagePath;
console.log('  Storage OK: ' + storageUrl);
