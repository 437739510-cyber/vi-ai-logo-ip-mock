#!/usr/bin/env node
// 一次性存量迁移：把 members.password_hash 中的旧明文改写为 scrypt 哈希。
// 幂等：已是哈希的行跳过；空/无密码行跳过（成员可能由 OTP 注册等路径产生，无 password_hash）。
// 默认 DRY-RUN（只列出，不写库）。
// 生产写库需满足两重确认：命令行 `--apply` 且环境变量 `MIGRATION_APPLY_CONFIRM=1`。
// 支持的离线 dry-run：`node scripts/migrate-member-passwords.mjs --dry-run --fixture <rows.json>`，
...
async function loadRows(opts) {
  ...
  const { data, error } = await client.from("members").select("id, phone, password_hash");
  if (error) {
    console.error("Failed to read members:", error.message);
    process.exit(1);
  }
...
function classify(rows) {
  ...
  const empty = [];
  for (const r of rows) {
    if (!r.password_hash || typeof r.password_hash !== "string" || r.password_hash.trim() === "") {
      empty.push(r);
    } else if (isPasswordHash(r.password_hash)) {
      alreadyHashed.push(r);
    } else {
      toMigrate.push(r);
    }
  }
  return { toMigrate, alreadyHashed, empty };
}

async function run() {
  const opts = parseArgs(process.argv.slice(2));
  const { rows, client } = await loadRows(opts);
  const { toMigrate, alreadyHashed, empty } = classify(rows);

  console.log("=== members.password_hash migration ===");
  console.log(
    `mode=${opts.apply ? "APPLY" : "DRY-RUN"}  total=${rows.length}  plaintext=${toMigrate.length}  alreadyHash=${alreadyHashed.length}  empty=${empty.length}`
  );
  console.log("");
  ...
  console.log(`empty/no-password rows (skipped): ${empty.length}`);
