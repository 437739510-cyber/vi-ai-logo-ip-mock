/**
 * TICKET-122-R3: frozen local POLP input, two real DeepSeek layout rounds,
 * deterministic baseline, PPTX structure/media identity evidence.
 *
 * Run only with the ticket-authorized local env loader:
 *   node --env-file=.env.local --import tsx scripts/_ab-real-deepseek-layout-122-r3.ts
 */
...
const BASE_OUT = path.join(ROOT, "logs/122-r3");
...
const plannerInput = Object.freeze({
  ...
  templateId: `__TICKET_122_R3_NO_TEMPLATE_${runId}__`,
  mascotAssets,
  assetAnalysis: {
...
async function injectWayfinding(pptx: Buffer, variant: Variant): Promise<Buffer> {
  ...
  const mediaName = "ppt/media/image-wayfinding-122-r3.png";
  ...
  const rel = `<Relationship Id="rId${nextId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image-wayfinding-122-r3.png"/>`;
  ...
  const pic = `<p:pic><p:nvPicPr><p:cNvPr id="9001" name="Wayfinding122R3"/><p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="rId${nextId}"/><a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="5290000" y="6995000"/><a:ext cx="1280160" cy="1280160"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr></p:pic>`;
