/**
 * TICKET-122-R4: frozen local POLP input, two real DeepSeek layout rounds,
 * deterministic baseline, PPTX structure/media identity evidence.
 *
 * Run only with the ticket-authorized local env loader:
 *   node --env-file=.env.local --import tsx scripts/_ab-real-deepseek-layout-122-r4.ts
 */
...
const BASE_OUT = path.join(ROOT, "logs/122-r4");
...
const plannerInput = Object.freeze({
  ...
  templateId: `__TICKET_122_R4_NO_TEMPLATE_${runId}__`,
  mascotAssets,
  assetAnalysis: {
...
const internalResourceEvidence: any[] = [];
...
globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
  ...
  if (rawUrl.startsWith("data:") || rawUrl.startsWith("blob:")) {
    const protocol = rawUrl.startsWith("data:") ? "data:" : "blob:";
    internalResourceEvidence.push({ protocol, method, internalResource: true, originalFetchCalled: true, at: new Date().toISOString() });
  ...
  try {
    ...
    blockedNetworkEvidence.push({ protocol: "invalid", method, stage: activeCall ? "paid-call" : "module-load", at: new Date().toISOString() });
    throw new Error("NETWORK_ALLOWLIST_BLOCK:invalid-url");
  }
  ...
  if (parsed.origin === SUPABASE_ORIGIN && parsed.pathname === "/rest/v1/api_usage_log") {
    ...
    supabaseStubEvidence.push({ origin: parsed.origin, path: parsed.pathname, method, stage, at: new Date().toISOString(), activeCall: activeCall ? { ...activeCall } : null, externalIo: false });
  ...
  const isExactDeepSeek = parsed.protocol === "https:" && parsed.hostname === "api.deepseek.com" && parsed.port === "" && parsed.pathname === "/v1/chat/completions" && parsed.search === "" && parsed.hash === "";
  if (!isExactDeepSeek) {
    blockedNetworkRequests += 1;
    blockedNetworkEvidence.push({ protocol: parsed.protocol, origin: parsed.origin, path: parsed.pathname, method, stage: activeCall ? "paid-call" : "module-load", at: new Date().toISOString(), activeCall: activeCall ? { ...activeCall } : null });
    throw new Error(`NETWORK_ALLOWLIST_BLOCK:${parsed.protocol}`);
  }
  ...
  try {
    ...
    record.requestThinkingType = requestBody.thinking?.type || null;
    record.requestThinkingDisabled = requestBody.thinking?.type === "disabled";
    record.requestModel = requestBody.model || null;
    record.requestMaxTokens = requestBody.max_tokens ?? null;
    record.requestTemperature = requestBody.temperature ?? null;
    record.requestMessageCount = Array.isArray(requestBody.messages) ? requestBody.messages.length : 0;
    record.completeRequestBodyRecorded = false;
    if (!record.requestThinkingDisabled) throw new Error("REQUEST_CONTRACT_MISSING_THINKING_DISABLED");
    ...
    record.contentIsNull = content === null || content === undefined;
    record.contentLength = typeof content === "string" ? content.length : 0;
    record.contentSha256 = typeof content === "string" && content.length > 0 ? sha256(content) : null;
    record.reasoningContentIsNull = reasoningContent === null || reasoningContent === undefined;
    record.reasoningContentLength = typeof reasoningContent === "string" ? reasoningContent.length : 0;
    record.reasoningContentSha256 = typeof reasoningContent === "string" && reasoningContent.length > 0 ? sha256(reasoningContent) : null;
    record.toolCallsCount = Array.isArray(message.tool_calls) ? message.tool_calls.length : 0;
...
PptxGenJS.prototype.addSlide = function tracedAddSlide(this: any, ...args: any[]): any {
...
async function injectWayfinding(pptx: Buffer, variant: Variant): Promise<Buffer> {
  ...
  const mediaName = "ppt/media/image-wayfinding-122-r4.png";
  ...
  const rel = `<Relationship Id="rId${nextId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image-wayfinding-122-r4.png"/>`;
  ...
  const pic = `<p:pic><p:nvPicPr><p:cNvPr id="9001" name="Wayfinding122R4"/><p:cNvPicPr><a:picLocks noChangeAspect="1"/></p:cNvPicPr><p:nvPr/></p:nvPicPr><p:blipFill><a:blip r:embed="rId${nextId}"/><a:stretch><a:fillRect/></a:stretch></p:blipFill><p:spPr><a:xfrm><a:off x="5290000" y="6995000"/><a:ext cx="1280160" cy="1280160"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></p:spPr></p:pic>`;
...
async function main(): Promise<void> {
  ...
  for (const round of ["A1", "A2"] as const) {
    for (const pageId of TARGET_PAGES) {
      ...
      while (!accepted && pageAttempts < MAX_ATTEMPTS_PER_PAGE) {
        ...
        const responseContractOk = Boolean(record &&
          record.obtainedHttpResponse &&
          record.httpStatus === 200 &&
          record.requestThinkingDisabled === true &&
          record.normalizedModel === "deepseek-v4-flash" &&
          record.usage &&
          record.finishReason === "stop" &&
          record.contentIsNull === false &&
          record.contentLength > 0 &&
          record.toolCallsCount === 0);
        if (record) record.responseContractOk = responseContractOk;
        if (!responseContractOk) continue;
  ...
  const evidence = {
    ...
    network: { allowedTarget: EXACT_DEEPSEEK_URL, deepseekNetworkRequests, blockedNetworkRequests, blockedNetworkEvidence, internalResourceRequests: internalResourceEvidence.length, internalResourceEvidence },
...
void main().catch((error) => {
  ...
  fs.writeFileSync(path.join(RUN_DIR, "fatal.json"), JSON.stringify({ runId, error: message, attempts: deepseekNetworkRequests, cumulativeCostCny, blockedNetworkRequests, blockedNetworkEvidence, internalResourceEvidence, supabaseStubEvidence, callEvidence }, null, 2), "utf8");
