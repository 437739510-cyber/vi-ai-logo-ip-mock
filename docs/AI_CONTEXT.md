...
Brand Brain Beta+ — 已完成分析 → 商业信息 → IP策略 → 套餐推荐 → 模块规划 → 生成计划 → 质量评分 → 记忆 完整闭环。
...
5. Mascot Designer 是新增 Agent — 插入 brand-planner → design-director 之间
6. Memory 是核心资产 — 客户记忆、行业知识、项目决策都存 JSON
7. Quality Score 是质量守门员 — 5 维度评分，低于 75 分标记 needs_revision

## 架构简述

```
Client Input → Brand Analyst → Brand Planner → Mascot Designer → Design Director → Asset Guardian → Manual Composer → Memory
```

7 个 Agent + Orchestrator，位于 `src/agents/`。
...
| `src/agents/mascot-designer.ts` | IP 策略 Agent（3 种模式） |
...
| `src/lib/mascot-prompt-strategy.ts` | IP 提示词生成引擎 |
| `src/lib/memory/` | 记忆系统 (types / json-adapter / index) |
| `src/components/admin/DecisionLayer.tsx` | 5 步决策向导 UI |
...
完善文档体系。候选：IP Creation Plan / Billing System / Style Extractor
